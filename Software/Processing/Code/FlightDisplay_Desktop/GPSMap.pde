...
void initGPSMap() {
  ...
  // Los tiles que falten se dibujan como celda gris
}
...
void updateGPSData() {
  ...
  // drawMapTrail no una los dos extremos con una recta
  if (lastFixState && !gpsFix && gpsTrail.size() > 0) {
    // Solo se inserta el marcador si el ultimo punto no es null
...
// Carga de tiles: lectura desde disco, con cache en RAM
// Devuelve null para tiles que no existen en disco (drawTiles pinta
// una celda gris en esos huecos). El null se cachea igual que un tile
// valido, asi no se llama a File.exists() en bucle
PImage getTile(int z, int x, int y) {
  ...
  String relativeBasePath = "../../Maps/data/tiles/"; 
  String tilePath = relativeBasePath + z + "/" + x + "/" + y + ".png";
  String fullPath = sketchPath(tilePath);
  PImage img = null;

  // El tile se valida antes de loadImage() para evitar mensajes de
  // "can't find file" en la consola cuando la coordenada cae fuera del
  // set descargado.
...
void drawGPSMap() {
  ...
  float mapH = baseHeight - 60;   // franja inferior para el status
...
void drawCompassRose(float x, float y) {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    fill(i == 0 ? color(200, 30, 30) : color(60));  // N en rojo
    noStroke();
    text(labels[i], tx, ty);
  }
...
void drawScaleBar(float x, float y) {
  ...
  // Lista de distancias, se elige la que mejor se aproxima
  // a ~150 px de longitud en pantalla
  float[] niceDistances = {
...
void drawZoomIndicator(float x, float y) {
  ...
  textSize(12);
  textAlign(LEFT, TOP);
  text("ZOOM: " + mapZoom + "  (" + MAP_ZOOM_MIN + "-" + MAP_ZOOM_MAX + ")",
       x, y);
...
void drawMapStatusStrip(float x, float y, float w) {
  ...
  text(nf(abs(gpsLat), 0, 5) + "\u00B0" + (gpsLat >= 0 ? "N" : "S"),
       x + 45, row1Y);

  fill(MAP_STRIP_LABEL);
  text("LON", x + 200, row1Y);
  fill(MAP_STRIP_VALUE);
  text(nf(abs(gpsLon), 0, 5) + "\u00B0" + (gpsLon >= 0 ? "E" : "W"),
       x + 235, row1Y);
...
float getTrackAngle() {
  // Localiza los dos ultimos puntos no nulos consecutivos
  float[] p1 = null;
  float[] p0 = null;
  for (int i = gpsTrail.size() - 1; i >= 0; i--) {
    float[] pt = gpsTrail.get(i);
    if (pt == null) {
      // Si hay un hueco entre los dos ultimos, el track no es calculable
  ...
  // Correccion de la longitud por latitud: a alta latitud un grado de
  // longitud equivale a menos metros que un grado de latitud.
...
// Llamado desde keyPressed() en FlightDisplay.pde cuando showMap == true
void handleMapKeys() {
  if (key == '+' || key == '=') {
    mapZoom = min(MAP_ZOOM_MAX, mapZoom + 1);
    tileCache.clear();  // un cambio de zoom invalida los tiles cargados
  }
  if (key == '-' || key == '_') {
